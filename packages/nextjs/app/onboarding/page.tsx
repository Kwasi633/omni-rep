...
type ProfileData = UserProfileData;
type IdentityData = UserIdentityData;
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    if (address && isConnected) {
      const isOnboarded = hasCompletedOnboarding(address);
      if (isOnboarded) {
      ...
      if (existingData) {
        setProfileData(prevProfile => ({ ...prevProfile, ...existingData.profile }));
        setIdentityData(prevIdentity => ({ ...prevIdentity, ...existingData.identity }));
      }
    }
  ...
  // Welcome screen with manual advancement
  useEffect(() => {
    if (currentStep === 'welcome') {
      // Auto advance from welcome after a longer delay to let users read, or let them click
  ...
  // No auto-advancement from connect to profile - user must click Continue
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      // Store onboarding completion and user data using the new system
      completeOnboarding(address, profileData, identityData);
    ...
    <div className="text-center space-y-8 max-w-2xl mx-auto">
      ...
      <div className="flex flex-col items-center space-y-4">
        <Button 
          ...
          className="bg-gradient-to-r from-teal-500 to-purple-500 px-8 py-3 text-lg font-semibold"
          size="lg"
        >
          Get Started
          {/* @ts-ignore */}
        ...
        <div className="flex items-center space-x-2 text-gray-500 text-sm">
          ...
          <span>Auto-advancing in a few seconds...</span>
        </div>
      </div>
    </div>
  );
          ...
          {isConnected && address && (
            <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4">
              ...
              <p className="text-green-300 text-xs mt-1">
                Ready to proceed to profile setup
              </p>
            </div>
          )}
        </div>
